	...
	class Puc_v4p6_Plugin_Ui {
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
			if ( $this->isMyPluginFile($pluginFile) ) {
				...
				$linkText = apply_filters(
					...
					__('استعلام آپدیت از نکسیمو', 'plugin-update-checker')
				);
				if ( !empty($linkText) ) {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('افزونه %s آخرین نسخه موجود در نکسیمو بوده و نیازی به آپدیت ندارد!', 'the plugin title', 'plugin-update-checker'), $title);
