...
function flatsome_header_nav( $nav, $walker = false ) {
	...
	<?php
		...
		echo '<li><a href="' . $admin_url . '">یک فهرست برای این قسمت انتخاب کنید از بخش > فهرست ها</a></li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function flatsome_get_header_html_element( $value ) {
	...
	if ( $value == 'html' ) {
		...
		$mod['default'] = '<strong class="uppercase">هر متنی تمایل دارید اینجا اضافه کنید، یا  کلا حذف کنید.</strong>';
	}
