...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			add_submenu_page('flatsome-panel', __( 'راه‌اندازی','envato_setup' ), __( 'راه‌اندازی','envato_setup' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
		}
		...
		?>
		<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'بارگشت به داشبورد وردپرس', 'envato_setup' ); ?></a>
		<a class="wc-return-to-dashboard" href="http://flatsomee.ir/"><?php _e( 'Flatsomee.IR', 'envato_setup' ); ?></a>
		</body>
		<?php
...
//				print_r($categories);
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( 'بزن بریم!' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'تمایلی ندارم' ); ?></a>
				</p>
				<?php
				...
				?>
				<h1><?php printf( __( 'به بخش درون ریزی دموهای قالب فلتسام خوش آمدید.' ), wp_get_theme()); ?></h1>
				<p class="lead"><?php printf( __( 'به بخش درون ریز دموهای قالب فلتسام خوش آمدید، در این بخش که توسط تیم ایران فلتسام به صورت کامل فارسی سازی شده، می توانید هر دمویی را که تمایل دارید درون ریزی کنید.' ), wp_get_theme()); ?></p>
				<p><?php _e( 'الان وقت ندارید؟ اگر نمی خواهید این مراحل را طی کنید ، می توانید رد شوید و به داشبورد وردپرس برگردید. اگر نظر خود را تغییر دادید می توانید هر زمان که دوست داشتید با استفاده از این بخش دموهای مورد علاقه تان را درون ریزی کنید.' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( 'بزن بریم!' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php _e( 'پلاگین های پیشفرض', 'envato_setup' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p class="lead"><?php _e( 'با این کار افزونه های پیش فرض موجود با Flatsome نصب می شود. می توانید پلاگین ها را بعداً از درون وردپرس اضافه و حذف کنید.', 'envato_setup' ); ?></p>
				...
				<p class="envato-setup-actions step">
					...
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'رد شدن از این مرحله', 'envato_setup' ); ?></a>
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php _e( 'نصب محتوای دمو', 'envato_setup' ); ?></h1>
			<form method="post">
				...
				<?php }else{ ?>
					<p class="lead"><?php printf( __( 'وقت آن است که برخی از مطالب پیش فرض را برای وب سایت جدید Flatsome خود وارد کنید. آنچه را که می خواهید در زیر وارد کنید انتخاب کنید و روی ادامه کلیک کنید. توصیه می شود همه چیز را انتخاب کنید. پس از درج ، این محتوا را می توان از داشبورد مدیر وردپرس مدیریت کرد. ', 'envato_setup' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
		...
		private function _make_child_theme( $new_theme_title ) {
				...
				if($parent_theme_template !== $new_theme_name){
					echo '<p class="lead success">نسخه چایلد قالب <strong>'.$new_theme_title.'</strong> ساخته و فعال شده است و در ادر wp-content/themes/ قرار گرفته است.<strong>'.$new_theme_name.'</strong></p>';
...
//									echo "Post meta $meta_key was $meta_val \n\n";
			...
			<form method="post">
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php _e( 'آپلود لوگو جدید', 'envato_setup' ); ?></a>
						</td>
					</tr>
				</table>
				<p>می توانید بعداً این گزینه را در گزینه های قالب بارگذاری و تنظیم کنید.</p>

				<hr/>

				<h3 style="margin-top: 30px;"><?php _e( 'دموی مورد نظر خود را انتخاب کنید :', 'envato_setup' ); ?></h3>
				<?php
				...
				<p><strong>تصاویر در دمویی که درون ریزی میکنید قرار ندارند. </strong><br/>* تصاویر گنجانده نشده است. شما باید تصاویر ساختگی را با تصاویر خود جایگزین کنید.
				تمام صفحات در محتوای نسخه ی نمایشی گنجانده شده است ، بنابراین شما این گزینه را در گزینه های تم مخلوط و مطابقت می دهید.
				</p>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php _e( 'فعال سازی قالب', 'envato_setup' ); ?></h1>
			<p class="lead">قالب شما توسط تیم ایران فلتسام (flatsomee.ir) فعال شده است، لذت ببرید.</p>
				<?php
				    ...
				    if ( ! empty( $purchase_code ) ) {
				      if ( ! empty( $errors ) ) {
				       ...
				       echo '<strong> توجه داشته باشید :</strong> در صورتیکه قالب را به صورت کپی و بدون خرید مستقیم از سایت ایران فلتسام (flatsomee.ir) دریافت کرده اید، وب سایت شما به زودی دچار مشکل خواهد شد. توصیه ما خرید مستقیم و قانونی از وب سایت ایران فلتسام است. <br><br>';
				      }
				    }
				    ...
				    echo '<form class="wupdates_purchase_code" action="" method="post">' .
				            ...
				            '<p style="margin-top: 15px">
				             ...
				             '<p class="envato-setup-actions step">' .
				              '<input type="submit" class="button button-large button-next button-primary" value="فعال سازی"/>' .
				              '<a href="'.esc_url( $this->get_next_step_link() ).'" class="button button-large button-next">'.__( 'رد شدن از این مرحله', 'envato_setup' ).'</a>'.
 				             '</p>
				      	</form>
				    ...
				    echo '<form class="wupdates_purchase_code" action="" method="post">' .
				             ...
				             '<input type="text" id="' . sanitize_title( $slug ) . '_wup_purchase_code" name="' . sanitize_title( $slug ) . '_wup_purchase_code"
				              ...
				              '<p class="envato-setup-actions step">' .
				              '<a href="'.esc_url( $this->get_next_step_link() ).'" class="button button-primary button-large button-next">'.__( 'ادامه', 'envato_setup' ).'</a>' .
 				             '</p>
				      </form>';
				  	}
?>
		...
		?>
			...
			<h1>تنظیم قالب فرزند فلتسام یا همان flatsome child (اختیاری)</h1>

			<p>
				توجه بفرمایید در پکیجی که از وب سایت ما دانلود کرده اید نسخه فلتسام چایلد موجود است، در صورتی که هنوز این  نسخه را بر روی وب سایت خود نصب نکرده اید  نصب کرده و بر روی سایت خود فعال کنید تا فونت های فارسی برروی سایت تان فعال شود.
			</p>

			<?php if(!isset($_REQUEST['theme_name'])){ ?>
			<p class="lead">اگر مطمئن نیستید که قالب فرزندچیست، فقط دکمه "پرش از این مرحله" را کلیک کنید.</p>
			<?php } ?>
			...
			<form action="<?php $_PHP_SELF ?>" method="POST">
			 ...
			 <label style="font-weight: bold;margin-bottom: 5px; display: block;">نام پوسته فرزند</label>
			...
			<p class="envato-setup-actions step">
		        <button type="submit" id= type="submit"  class="button button-primary button-next button-next">
		          <?php _e( 'ساختن و استفاده از قالب چایلد', 'envato_setup' ); ?>
		        </button>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'رد شدن از این مرحله', 'envato_setup' ); ?></a>
			...
			<?php } else { ?>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'ادامه', 'envato_setup' ); ?></a>
			</p>
			<?php } ?>
			<?php
		}
		public function envato_setup_help_support() {
			?>
			<h1>پشتیبانی در ایران فلتسام</h1>
			<p class="lead">تمامی کاربرانی که قالب فلتسام را از سایت ایران فلتسام تهیه کرده اند، از 6 ماه پشتیبانی بهره مند هستند. جهت ارتباط با بخش پشتیبانی می توانید وارد صفحه تماس با ما سایت مان شده و از طریق یکی از راه های ارتباطی که ذکر شده با ما در ارتباط باشید.</p>
		...
		public function envato_setup_ready() {
			...
			<p class="lead success">تبریک می گویم! قالب شما آماده است. حالا می توانید از تمامی امکانات این قالب وردپرس استفاده کرده و لذت ببرید. از این که به ایران فلتسام اعتماد کردید متشکریم.</p>
